...
module.exports.createProject = async function (projectData) {
    let result = requestValidation.isValidBody(["projectName", "number", "createdAt", "numStations", "numRuns", "timePerRun", "productionTarget", "status"], projectData);
...
module.exports.getProject = async function(projectId) {
    return projectModel.findByProjectId(projectId).then(function (project) {
        if(!project) {
            errorUtil.createAndThrowGenericError(`Could not find the project specified by id: ${projectId}`, 404);
        } else {
            return project;
        }
    });
